...
| `metadata_cache_enabled` | `bool` | `false` (requires `metadata-cache`) | Java `goosefs.user.metadata.cache.enabled`. The `metadata-cache` crate feature still compiles the cache in (`cfg!(feature = "metadata-cache")`); the runtime switch defaults to `false`. When `true`, `get_status` / `list_status` / `exists` / open share one process-local LRU (status + listing + negative cache). Writes invalidate path + parent after a successful RPC. Every reader open resolves its `FileInfo` through this cache, so with it off a workload of many small ranged reads (one reader per read, as OpenDAL does) pays one Master `get_status` RPC per read. Enable it when those RPCs dominate; leave it off when the file set mutates behind the client faster than `metadata_cache_expiration`. |
...
it per call via `ListStatusOptions.load_metadata_type` /
`GetStatusOptions.load_metadata_type` over setting it globally.
...
`ALWAYS` does **not** skip the client **status** cache. To force every
`get_status` / `exists` / open to the Master, use
`file_metadata_sync_interval = 0` or `GetStatusOptions::always_sync()`.
...
| `GOOSEFS_METADATA_CACHE_ENABLED` | `metadata_cache_enabled` | `false` | Enable the client metadata cache (`true`/`false`/`1`/`0`). |
...
let config = GoosefsConfig::new("10.0.0.1:9200")
    ...
    // Metadata cache: off by default (Java-aligned). Enable it to skip
    // repeated Master get_status RPCs. TTL 10min / capacity 100000 match Java.
...
| `metadata_cache_enabled` | Repeated opens / get_status / list_status of the same paths | `false` | `true` (requires `metadata-cache`; leave `false` when the file set mutates behind the client) | `GOOSEFS_METADATA_CACHE_ENABLED` | `goosefs.user.metadata.cache.enabled` | `goosefs_metadata_cache_enabled` |
